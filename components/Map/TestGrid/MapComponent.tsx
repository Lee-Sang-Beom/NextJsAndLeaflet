...
// @ts-ignore
import { gimhaeGrid } from "./populationDensityData";
...
const MapComponent = () => {
  ...
  const [gimhaeGridList, setGimhaeGridList] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    // 5179 -> 4326
    const gridData = gimhaeGrid.features
      .filter((item: any) => {
        return item.properties.val && item.properties.val > 0;
      })
      .map((item: any) =>
        item.geometry.coordinates.map((item2: any) =>
          item2.map((item3: any) => {
            return {
              lat: item3[1],
              lng: item3[0],
    ...
    console.log("grid is ", gridData);
    setGimhaeGridList(gridData);
  }, []);
  ...
  function GridPolygon() {
    const allPathOption = { stroke: true, color: "red", fillColor: "red" };

    return (
      <>
        {gimhaeGridList.map((grids, idx1) => {
          return grids.map((grid: any, idx2: number) => {
            return (
              <Polygon
                key={idx2}
                interactive={false}
